  ...
  return (
    <nav className="flex-grow md:space-x-6 flex flex-col md:flex-row md:justify-end md:items-center space-y-1 md:space-y-0">
      {NavItems.map(({ link, name }, index) => {
        return (
          <Link className="cursor-pointer font-manrope font-medium " href={link} key={index}>
            {name}
          </Link>
        );
      })}
  ...
  return (
    <>
      <header
        ...
        className={`flex justify-between overflow-hidden z-[1200] sticky  top-0 ${router?.pathname == "/"
          ? "bg-white text-black  "
          : "bg-white"
          } items-center px-6 py-2 ${router?.pathname == "/"
            ? "overflow-hidden "

...
import ProjectsCard from "src/componets/HousCard/ProjectsCard";
...
export default function Home() {
  ...
  const [featureFiltred, setFeatureFiltred] = useState<Propery[] | null>([]);
  // const [costFiltred, setCostFiltred] = useState<Propery[] | null>([]);
  const [costFiltred, setCostFiltred] = useState([]);
  ...
  const allProperties = [/* your array of properties here */];


  if (featureFiltred) {
    console.log(featureFiltred, "dfdsfsfdsfsdfsdf")
  }
  ...
  //     if (featured?.result) {
  //       // setFiltred(featured?.result);
  //       setFeatureFiltred(featured?.result);
  //     }
  //   } else {
  //     console.log(propertyTypeFilter,"jdjkgfgjk")
  //     setFeatureFiltred((prev) => {
  //       const arr = featured?.result?.filter((p) => {
  //         return p.propertyType == propertyTypeFilter;
  //       });
  //       return arr as Propery[];
  ...
  useEffect(() => {
    if (propertyTypeFilter === "all") {
      ...
      console.log(propertyTypeFilter, "jdjkgfgjk");
      const arr = featured?.result?.filter((p) => p.propertyType.toLowerCase() === propertyTypeFilter.toLowerCase());
      console.log(arr, "skdjfklsdjkf")
      setFeatureFiltred(arr || []);
    }
  }, [propertyTypeFilter, featured?.result]);

  // Filter functions

  useEffect(() => {
    console.log("Filtred in useEffect:", Filtred);

    const filterProperties = () => {
      // Check if Filtred is available and has length
      if (Filtred && Filtred.length > 0) {
        const filteredProperties = Filtred.filter(
          (property) => property.cost < 5000000
        );
        console.log("Filtered properties by cost:", filteredProperties);
        setCostFiltred(filteredProperties);

        const featuredProperties = Filtred.filter(
          (property) => property.featured
        );
        console.log("Featured properties:", featuredProperties);
        setFeaturedFiltred(featuredProperties);

        const toggleProperties = Filtred.filter(
          (property) => property.toggle === "Project"
        );
        console.log("Toggle properties:", toggleProperties);
      ...
      <div>
        ...
        <section className="pb-16 px-5 md:px-10  bg-[#F4F4F4] pt-12 w-full">
          ...
          <div className="relative max-w-7xl mx-auto px-5 md:px-10">
            <div className="flex space-x-4 pt-10 items-center overflow-x-auto">
              {homeChipsData.map(({ name, value, Icon }) => {
                return (
                  <div
                    key={name}
                    onClick={() => {
                      console.log(value, "ljfdksjfskd")
          ...
          <div className="max-w-7xl mx-auto px-5 md:px-10" >
            {featureFiltred?.length ? (
              <CardCarousel id="house" data={featureFiltred} Card={HouseCard} />
            ) : (
        ...
        {/* Projects card created by me */}
        {/* <section style={{border:"2px solid red"}} className="py-4">
          <div className="max-w-7xl mx-auto px-5 md:px-10 ">
            <div className="w-full flex items-center justify-between">
              <HomeSectionTitle text="Projects" />
              <div className="hidden md:flex space-x-4 ">
                <button
                  onClick={() => scrollLeft("projects")}
                  className="p-2 m-2 rounded-full bg-white"
                >
                  <FiChevronLeft />
                </button>
                <button
                  onClick={() => scrollRight("projects")}
                  className="p-2 m-2 rounded-full bg-white"
                >
                  <FiChevronRight />
                </button>

...
const Property = () => {
  ...
  async function getAllUsers() {
    ...
    try {
      ...
      const res = await instance.get(
        // `/admin/getAllPropertiesByAdmin?search=${name || ""}&toggle=${selected}`
  ...
  async function deleteCustomer() {
    try {
      ...
      const res = await instance.delete("/admin/property/deletePropertybyadmin", { data: { propertyId: deleteId } });
      if (res.data) {
  ...
  function handleFeatured(value: string, id: string) {
    console.log(value, id, "fhdsjshjjd")
    setUsers((prev) =>
      prev.map((selectedExercise) => {
        if (selectedExercise._id === id) {
          return {
    ...
    onFeaturedSubmit({ featured: value, propertyId: id });
  }
  ...
  async function onFeaturedSubmit(data: propertyStatus) {
    try {
      setLoading(true);
      const res = await instance.put(
        "/admin/property/changePropertyFeatured",
        data
      );
      if (res.data) {
        toast.success("Property updated Successfully");
    ...
    },
    {
      ...
      renderCell: ({ row }) => (
        <Select
          fullWidth
          size="small"
          onChange={(e) => handleFeatured(e.target.value, row?._id)}
          value={row?.featured}
        >

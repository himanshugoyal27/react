...
const AddProperty = ({ propertyData, navbarFooter, isEdit }) => {
  ...
  const handlePostProperty = async () => {
    ...
    bodyFormData.append('size', String(size));
    ...
    bodyFormData.append('areaValue', String(areaValue));
    ...
    bodyFormData.append('metaTittle', metaTittle);
    bodyFormData.append('metaDescription', metaDescription);
  ...
  const handlePropertiesDetails = () => {
    ...
    //   handleNext();
    // } else {
    console.log(price, size, "sdfhjskdhfds")
    ...
    //   return;
    // }
    if (price === '' || price === null || price === undefined || parseFloat(price) === 0) {
      setPriceError('Please fill the price');
    ...
    //   setSizeError('Area cannot be negative');
    //   setTimeout(() => {
    //     setSizeError('');
    //   }, 2000);
    //   return;
    // }
    if (size === '' || size === null || size === undefined || parseFloat(size) === 0) {
      setSizeError('Please enter a valid number for the area');
    ...
    if (areaValue.trim() === '') {
      ...
      return;
    } else if (parseFloat(areaValue) < 0) {
      setAreaValueError('Area value cannot be negative');
      setTimeout(() => {
        setAreaValueError('');
      }, 2000);
      return;
  ...
  const handleAdditionalDetails = () => {
    ...
    if (
      ...
      if (floorNumber.trim() === '') {
        ...
        return;
      } else if (parseFloat(floorNumber) < 0) {
        setFloorNumberError('Floor number cannot be negative');
        setTimeout(() => {
          setFloorNumberError('');
        }, 2000);
        return;
    ...
    if (
      ...
      if (tower.trim() === '') {
        ...
        return;
      } else if (parseFloat(tower) < 0) {
        setTowerError('Tower number cannot be negative');
        setTimeout(() => {
          setTowerError('');
        }, 2000);
        return;
    ...
    if (
      ...
      if (floorCount.trim() === '') {
        ...
        return;
      } else if (parseFloat(floorCount) < 0) {
        setFloorCountError('Floor count cannot be negative');
        setTimeout(() => {
          setFloorCountError('');
        }, 2000);
        return;
    ...
    if (
      ...
      if (unitNumber.trim() === '') {
        ...
        return;
      } else if (parseFloat(unitNumber) < 0) {
        setUnitNumberError('Unit number cannot be negative');
        setTimeout(() => {
          setUnitNumberError('');
        }, 2000);
        return;
  ...
  useEffect(() => {
    ...
    if (propertyData) {
      ...
      if (propertyData?.availableFor === "rent") {
        // setDevelopmentActive(true)
        // setDevelopmentActive(true);
        setRentActive(true);
        setSaleActive(false);
      ...
      if (propertyData?.availableFor === "sale") {
        // setDevelopmentActive(true)
        // setDevelopmentActive(true);
        setRentActive(false);
        setSaleActive(true);
      ...
      if (propertyData?.toggle === "Project") {
        setPropertyActive(false);
        setProjectActive(true);
        setToggle("Project");
      } else {
      ...
      setSelectedPropertyFeatures(propertyData?.propertyTags)
      setMetaTittle(propertyData?.metaTittle)
      setMetaDescription(propertyData?.metaDescription)

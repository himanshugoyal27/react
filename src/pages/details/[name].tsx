...
const SpecificationItem = ({ Icon, text, tagName }) => {
  return (
    <div className="md:w-[400px] w-full flex md:items-center md:justify-start items-center justify-start my-[10px]" >
      <div className="md:w-[200px] flex items-center justify-start" >
        <Icon />
        <p className="md:m-0 md:ml-[10px] ml-[10px]  ">{tagName}:</p>
      </div>
      <p className="md:m-0 md:ml-[40px] ml-[10px]  ">{text}</p>
    </div>
  );
};
...
const Details = () => {
  ...
  const [contactFlag, setContanctFlag] = useState(false)
  ...
  const { data: checkProperty } = useFetch(
    `/property/myPropertyAgent/${id}`
  );
  ...
  useEffect(() => {
    if (checkProperty?.data) {
      setContanctFlag(checkProperty?.data)
    }
  }, [checkProperty]);
  ...
  //       if (id) {
  //         console.log(id, "iddddd")
  //         const res = await instance.post("/property/myPropertyAgent/" + id);
  //         if (res.data.data) {
  //           // toast.success("Property added in Favourite");
  //           console.log(res.data.data, "jdhfsjksdfhkdjs");
  //         }
  //       }
  //     } else {
  //       // toast("Please login in order to save");
  //       setContanctFlag(true)
  //       // router.push("/login");
  //     }
  //   } catch (error) {
  //     console.error("An error occurred:", error);
  ...
  const handleAgentContact = async () => {
    try {
      if (cookies.jwtToken) {
        ...
        if (resContactAgent.data) {
          ...
          const resAddLead = await instance.post(
            "/leads/addLead",
            {
              adminId: "6549e15fec04ff59f1eba3dc",
              propertyId: id,
      ...
      console.error("An error occurred:", error);
      toast.error("You have already contacted the agent for this property");
    }
  }
      ...
      <main className=" py-12 px-5 md:px-8 space-y-6 max-w-7xl mx-auto w-full">
          ...
          <div className=" md:flex justify-between items-start space-y-4">
            ...
            <div className="flex flex-col ">
              ...
              >
                ...
                {contactFlag ? "" : <div onClick={handleFavourite} className="flex items-center space-x-1 border px-3 py-1 rounded-full bg-white/70 cursor-pointer shadow-sm active:scale-105 transition transform duration-200 active:bg-gray-100">
        ...
        <section className="space-y-10">
              ...
              <>
                {contactFlag ? (
                  <button
                    // onClick={handleAgentContact}
                    style={{ borderRadius: "20px" }}
                    className={`bg-${buttonColor ? "current" : "primaryBlue"} text-white w-full py-2 rounded-sm shadow-sm hover:opacity-95 active:opacity-80 transition transform duration-200 ease-out`}
                  >
                    Your Property
                  </button>
                ) : (
            ...
            <div className="md:flex md:space-x-4 space-y-3 md:space-y-0 font-manrope">
              <div className="flex w-full justify-between items-start flex-col md:flex-row md:space-x-4 space-y-3 md:space-y-0 font-manrope">
                ...
                <div className="leftSideContainer">
                  ...
                  {data?.result.propertyType && <SpecificationItem Icon={FaHome} tagName={"Property Type"} text={data?.result.propertyType} />}
                  {data?.result.buildingType && <SpecificationItem Icon={FaBuilding} tagName={"Building Type"} text={data?.result.buildingType} />}
                  {data?.result.userType && <SpecificationItem Icon={FaUser} tagName={"User Type"} text={data?.result?.userType} />
                  }

                  {data?.result.availableFor && <SpecificationItem Icon={FaRegBookmark} tagName={"Available for"} text={data?.result.availableFor} />}

                  {data?.result.ageOfProperty && <SpecificationItem Icon={FaRegBookmark} tagName={"Age of the Property"} text={data?.result.ageOfProperty} />}

                  {data?.result.size && <SpecificationItem Icon={FaRuler} tagName={"Area"} text={data?.result.size} />
                  }
                  {data?.result.liftFacility && <SpecificationItem Icon={GiLift} tagName={"Lift Facility"} text={data?.result.liftFacility} />
                  }
                  {data?.result.toggle && <SpecificationItem Icon={FaBuilding} tagName={"Toggle"} text={data?.result.toggle} />
                  }
                  {data?.result.facing && <SpecificationItem Icon={FaBuilding} tagName={"Facing"} text={data?.result.facing} />
                  }
                  {data?.result.floorCount && <SpecificationItem Icon={FaBuilding} tagName={"Floor Count"} text={data?.result.floorCount} />
                  }
                ...
                <div className=" md:pr-[300px]">
                  {data?.result.authority && <SpecificationItem Icon={FaRegBookmark} tagName={"Authority"} text={data?.result.authority} />
                  }
                  {data?.result.BHKconfig && <SpecificationItem Icon={FaBed} tagName={"BHK Configure"} text={data?.result.BHKconfig} />
                  }
                  {data?.result.additionalRooms && <SpecificationItem Icon={FaRegBookmark} tagName={"Additional Rooms"} text={data?.result.additionalRooms} />}

                  {data?.result.possessionStatus && <SpecificationItem Icon={FaRegBookmark} tagName={"Possession Status"} text={data?.result.possessionStatus} />}

                  {data?.result.furnishingStatus && <SpecificationItem Icon={GiHandTruck} tagName={"Furnishing Status"} text={data?.result.furnishingStatus} />}

                  {data?.result.floorNo && <SpecificationItem Icon={FaBed} tagName={"Floor No."} text={data?.result.floorNo} />
                  }
                  {data?.result.numOfBathroom && <SpecificationItem Icon={MdBathroom} tagName={"No. of Bathroom"} text={data?.result.numOfBathroom} />
                  }
                  {data?.result.numOfParking && <SpecificationItem Icon={FaParking} tagName={"No. of Parking"} text={data?.result.numOfParking} />
                  }
                  {data?.result.view && <SpecificationItem Icon={FaBuilding} tagName={"View"} text={data?.result.view} />
                  }
                  {data?.result.towerBlock && <SpecificationItem Icon={FaBuilding} tagName={"Tower Block"} text={data?.result.towerBlock} />
                  }
                  {data?.result.unitNo && <SpecificationItem Icon={FaRuler} tagName={"Unit No."} text={data?.result.unitNo} />
                  }
                </div>
              </div>
            </div>
          ...
          >
            ...
            <div className="md:flex md:space-x-4 space-y-3 md:space-y-0 font-manrope overflow-x-auto scrollbar-hide">

...
const CustomPopup =  ({closePopup}) => {
  ...
  const [tittle, setTittle] = useState("Assign Agent")
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await instance.post("/user/ticket/addTicket", formData);
...
const Ticket = () => {
  ...
  const { data, status } = userDetails ? useFetch<response<Tickets[]>>(
    `/user/ticket/getTicketByUserId/${userDetails._id}`) : { data: null, status: 'idle' };
